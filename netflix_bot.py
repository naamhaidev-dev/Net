...
# Only use get_emoji() for message text with parse_mode="HTML", not for button labels.
PREMIUM_EMOJIS = {
    "💎": "5427168083074628963",
    "🥇": "5440539497383087970",
    "⌛": "5386367538735104399",
    ...
    "🔴": "6104873438021686768",
    "👥": "5987557724886405444",
    "📊": "5316977222467206948",  # custom mapping for bar chart
    "📁": "5427168083074628963",  # fallback
}
...
def build_login_keyboard(cookie_id: str, nftoken: str) -> InlineKeyboardMarkup:
    ...
    Note: Button labels use plain emoji characters (not HTML tags) because button text is plain.
    """
    keyboard = [
        [
            InlineKeyboardButton("💻 PC", url=f"https://www.netflix.com/login?nftoken={nftoken}"),
            InlineKeyboardButton("📺 TV", url=f"https://www.netflix.com/tv8?nftoken={nftoken}"),
        ],
        [
            InlineKeyboardButton("📱 Android", url=f"https://www.netflix.com/android?nftoken={nftoken}"),
            InlineKeyboardButton("🍏 iPhone", url=f"https://www.netflix.com/iphone?nftoken={nftoken}"),
        ],
        [
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        await update.message.reply_text(
            f"{get_emoji('❌')} <b>User not found</b>",
            parse_mode="HTML"
        )
        return
...
async def handle_bulk_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not document:
        await update.message.reply_text(
            f"{get_emoji('❌')} <b>No file received</b>",
            parse_mode="HTML"
        )
        return BULK_CHECK_STATE
    ...
    if not document.file_name.endswith('.txt'):
        await update.message.reply_text(
            f"{get_emoji('❌')} <b>Only .txt files allowed</b>",
            parse_mode="HTML"
        )
        return BULK_CHECK_STATE
