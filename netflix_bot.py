...
SKIP_VALID_RECHECK = os.getenv("SKIP_VALID_RECHECK", "False").lower() == "true"
...
PREMIUM_EMOJIS = {
    ...
    "📍": "5391032818111363540",
    "🔗": "5271604874419647061",
    "🖥️": "5323440478232783499",
    "🎧": "6007938409857815902",
    "👁️": "5960714428394507968",
    "🐍": "5260480440971570446",
    "📦": "5301137237050663843",
    "🦇": "5811901387910418000",
    "👑": "5413879192267805083",
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get("https://www.netflix.com/in/", headers=headers, timeout=timeout, ssl=None, allow_redirects=True) as resp:
                    ...
                    if not result["profile_name"]:
                        result["profile_name"] = PlatformChecker._extract_field(html, "profileName") or "Unknown"
...
# Handle direct cookie message (for backward compatibility – ask to use /check)
...
async def handle_upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # This handler is for /upload command, expects platform argument and document attachment
    user_id = update.effective_user.id
    if not db.is_user_approved(user_id) and user_id not in ADMIN_IDS:
        await update.message.reply_text(
            f"{get_emoji('⛔️')} <b>Access Denied</b>",
    ...
    await process_zip(update, context, platform, doc)

async def process_zip(update: Update, context: ContextTypes.DEFAULT_TYPE, platform: str, document):
    user_id = update.effective_user.id
    msg = await update.message.reply_text(
...
# Handle document upload without /upload command – ask user to use command
async def handle_document_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"{get_emoji('❌')} Please use <code>/upload &lt;platform&gt;</code> command with .zip file.",
        parse_mode="HTML"
    )
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if db.approve_user(user_id):
            ...
            await update.message.reply_text(
                f"{get_emoji('❌')} Failed to approve user.",
                parse_mode="HTML"
            )
    except ValueError:
        await update.message.reply_text(
            f"{get_emoji('❌')} Invalid user ID.",
            parse_mode="HTML"
        )
...
async def admin_disapprove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if db.disapprove_user(user_id):
            ...
            await update.message.reply_text(
                f"{get_emoji('❌')} Failed to disapprove user.",
                parse_mode="HTML"
            )
    except ValueError:
        await update.message.reply_text(
...
def main():
    ...
    application.add_handler(CommandHandler("upload", handle_upload_command))
    ...
    # Document handler – only if it's not a command (i.e., direct file upload)
    ...
    logger.info("Multi-Platform Cookie Checker Bot started successfully!")
