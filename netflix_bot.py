...
import uuid
import tempfile
import urllib.parse  # <-- FIX: added for URL encoding/decoding
...
# ----------------------------- FIX: Parser preserves spaces in values -----------------
def parse_netscape_cookie(cookie_text: str) -> Dict[str, str]:
    """
    Parse Netscape cookie file.
    FIX: join all remaining parts after the name to preserve spaces/parentheses.
    """
    cookie_dict = {}
    ...
    for line in lines:
        ...
        if len(parts) >= 7:
            name = parts[5]
            # The value is everything from index 6 onward (may contain spaces)
            value = ' '.join(parts[6:])
...
# ----------------------------- FIX: URL-encode nftoken in login links ------------
def build_login_keyboard(cookie_id: str, nftoken: str) -> InlineKeyboardMarkup:
    """
    Build inline keyboard with device login buttons.
    FIX: URL-encode the token to handle +, /, = etc.
    """
    encoded = urllib.parse.quote(nftoken, safe='')   # encode all special chars
    keyboard = [
        [
        ...
        [
            InlineKeyboardButton("📱 Login Android", url=f"https://netflix.com/android?nftoken={encoded}"),
            InlineKeyboardButton("🍏 Login iPhone", url=f"https://netflix.com/iphone?nftoken={encoded}"),
        ],
        [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/login `<cookie_id>` - Generate device login link (nftoken)\n"
...
async def check_single(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build response with inline keyboard if valid and token exists
    if result.get("valid"):
        response = (
            "✅ *Account is Active*\n\n"
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("❌ Usage: /login <cookie_id>")
        return
    cookie_id = args[0]
    cookie_doc = db.get_cookie(cookie_id)
    if not cookie_doc:
        await update.message.reply_text("❌ Cookie not found.")
        return
    if cookie_doc.get("status") != "valid":
        await update.message.reply_text(f"❌ Cookie is not valid (status: {cookie_doc.get('status')})")
        return
    raw = denormalize_cookie(cookie_doc.get("cookie", ""))
    nftoken = extract_nftoken(raw)
    if not nftoken:
        await update.message.reply_text("❌ Could not extract nftoken from this cookie.")
        return
    # Build keyboard
    keyboard = build_login_keyboard(cookie_id, nftoken)
    response = (
        ...
        f"Tier: {cookie_doc.get('account_tier', 'Unknown')}\n\n"
        f"Choose a device:"
    )
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("upload_"):
        ...
        return
    # If it's a login button, we already have url links, so no need to handle here
...
async def handle_bulk_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not valid_cookies:
            ...
            return ConversationHandler.END
        await msg.edit_text(f"✅ Added {len(valid_cookies)} cookies from file.\n❌ Invalid/skipped: {len(cookies) - len(valid_cookies)}\n\n🔄 Starting validation...")
        ...
        db.update_user_activity(user_id)
        response = f"📊 *Bulk Check Results*\n\nTotal processed: {len(results)}\n✅ Valid: {valid_count}\n❌ Invalid/Expired: {len(results) - valid_count}"
