...
async def handle_cookie_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Run checks on all blocks
        results = await NetflixChecker.check_bulk_cookies(blocks, progress_callback=progress_callback)
        ...
        for result in results:
            ...
            if result.get("valid"):
                ...
                # Still log check? Not necessary since we don't have an ID.
                # We'll just count it.

        # Update user stats
        db.increment_user_stats(user_id, total_blocks, len(valid_list))
        db.update_user_activity(user_id)
