...
# ----------------------------- NETFLIX CHECKER ENGINE (REVISED) -----------------
class NetflixChecker:
    @staticmethod
    async def check_single_cookie(cookie: str) -> Dict:
        """
        Check a single Netflix cookie by hitting the homepage.
        This is more reliable than API calls which require extra headers.
        """
        result = {
            "valid": False,
            "account_tier": None,
        ...
        # Try proxies; fallback to no proxy if all fail
        proxies_to_try = []
        if USE_PROXY:
            # Pick up to 3 random proxies
            for _ in range(min(3, len(_all_proxies))):
                p = get_random_proxy()
                if p and p not in proxies_to_try:
                    proxies_to_try.append(p)
        proxies_to_try.append(None)  # fallback no proxy
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(
                        ...
                        # Check if logged in
                        # Indicators: redirect to /browse, /profiles, presence of "logout" in HTML
                        logged_in = False
                        if "logout" in html_lower:
                            logged_in = True
                        elif "/browse" in final_url or "/profiles" in final_url:
                            logged_in = True
                        elif "login" not in final_url and "signin" not in final_url:
                            # If not redirected to login and HTML doesn't have sign-in prompts
                            if "sign in" not in html_lower and "signin" not in html_lower:
                                logged_in = True
                        ...
                        # Logged in: extract tier and profile
                        result["valid"] = True
        ...
        # If after all proxies still invalid, set error
    ...
    def _extract_tier(html: str) -> str:
        """Extract account tier from HTML using various patterns"""
        # Look for plan name in profile dropdown
        ...
        if match:
            ...
            if "premium" in plan:
                ...
                return "Basic"
        # Fallback to maturity levels count heuristic
    ...
    def _extract_profile_name(html: str) -> str:
        ...
        # Look for profile name in profile dropdown
