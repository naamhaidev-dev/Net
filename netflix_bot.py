...
def build_login_keyboard(cookie_id: str, nftoken: str) -> InlineKeyboardMarkup:
    keyboard = [
        [
            InlineKeyboardButton(f"{get_emoji('🖥️')} Login PC", url=f"https://netflix.com/login?nftoken={nftoken}"),
            InlineKeyboardButton(f"{get_emoji('📺')} Login TV", url=f"https://netflix.com/tv8?nftoken={nftoken}"),
        ],
        [
            InlineKeyboardButton(f"{get_emoji('📱')} Login Android", url=f"https://netflix.com/android?nftoken={nftoken}"),
            InlineKeyboardButton(f"{get_emoji('🍏')} Login iPhone", url=f"https://netflix.com/iphone?nftoken={nftoken}"),
        ],
        [
            InlineKeyboardButton(f"{get_emoji('📤')} Upload File", callback_data=f"upload_{cookie_id}"),
            InlineKeyboardButton(f"{get_emoji('🔄')} Restart", callback_data="restart"),
        ]
    ]
...
def admin_required(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if user_id not in ADMIN_IDS:
            await update.message.reply_text(f"{get_emoji('⛔️')} <b>Unauthorized</b>\nYou are not allowed to use this command.")
            return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"  • /bulk – Upload .txt file (one per line)\n"
...
async def check_single(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result.get("valid"):
        ...
        response += f"{get_emoji('🆔')} <b>Cookie ID:</b> <code>{cookie_id[:8]}</code>\n"
        ...
        response = (
            ...
            f"{get_emoji('🆔')} <b>Cookie ID:</b> <code>{cookie_id[:8]}</code>"
        )
        await msg.edit_text(response, parse_mode="HTML")
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cookie_doc:
        await update.message.reply_text(
            ...
            f"Cookie ID <code>{cookie_id}</code> does not exist.",
            parse_mode="HTML"
        )
        return
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        await update.message.reply_text(f"{get_emoji('❌')} <b>User not found</b>", parse_mode="HTML")
        return
    ...
    response = (
        f"{format_header('Your Statistics', '📊')}\n"
        f"{SEP}\n"
        f"{get_emoji('📈')} <b>Total Checks:</b> <code>{user.get('total_checks', 0)}</code>\n"
        f"{get_emoji('✅')} <b>Valid Cookies:</b> <code>{user.get('valid_cookies', 0)}</code>\n"
        f"{get_emoji('📅')} <b>Since:</b> <code>{user.get('created_at', get_current_time()).strftime('%Y-%m-%d')}</code>\n\n"
        f"{format_header('Global Stats', '🌐')}\n"
        f"{SEP}\n"
        f"{get_emoji('🍪')} <b>Total:</b> <code>{stats_data['total']}</code>\n"
        f"{get_emoji('✅')} <b>Valid:</b> <code>{stats_data['valid']}</code>\n"
        f"{get_emoji('❌')} <b>Invalid:</b> <code>{stats_data['invalid']}</code>\n"
        f"{get_emoji('⏰')} <b>Expired:</b> <code>{stats_data['expired']}</code>\n"
        f"{get_emoji('⏳')} <b>Pending:</b> <code>{stats_data['pending']}</code>\n\n"
        f"{format_header('Tier Distribution', '⭐')}\n"
        f"  {get_emoji('⭐')} Premium: <code>{stats_data['tiers']['Premium']}</code>\n"
        f"  {get_emoji('📺')} Standard: <code>{stats_data['tiers']['Standard']}</code>\n"
        f"  {get_emoji('📱')} Basic: <code>{stats_data['tiers']['Basic']}</code>\n"
    )

    if user_checks:
        response += f"\n{get_emoji('🕒')} <b>Recent Checks</b>\n"
        for check in user_checks[:5]:
            status_emoji = get_emoji('✅') if check.get('status') == 'valid' else get_emoji('❌')
            timestamp = check.get('timestamp', get_current_time()).strftime("%H:%M")
            response += f"{status_emoji} {timestamp} – <code>{check.get('status')}</code>\n"
...
async def bulk_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{format_header('Bulk Cookie Check', '📁')}\n"
        f"{SEP}\n"
        f"{get_emoji('📤')} Please upload a <b>.txt</b> file with one cookie per line.\n"
        f"{get_emoji('⚠️')} Max: <code>{MAX_BULK_CHECK}</code> cookies per batch.\n"
        f"Supported formats: Standard, Netscape, Base64.\n\n"
...
async def handle_bulk_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not document:
        await update.message.reply_text(f"{get_emoji('❌')} <b>No file received</b>", parse_mode="HTML")
        return BULK_CHECK_STATE

    if not document.file_name.endswith('.txt'):
        await update.message.reply_text(f"{get_emoji('❌')} <b>Only .txt files allowed</b>", parse_mode="HTML")
        return BULK_CHECK_STATE

    msg = await update.message.reply_text(
        f"{get_emoji('📥')} <b>Downloading file...</b>",
        parse_mode="HTML"
    )
    ...
    try:
        ...
        if not cookies:
            await msg.edit_text(f"{get_emoji('❌')} <b>Empty file</b>\nNo cookies found.", parse_mode="HTML")
            return ConversationHandler.END
        ...
        await msg.edit_text(
            f"{get_emoji('✅')} <b>Added {len(valid_cookies)} cookies</b>\n"
            f"{get_emoji('❌')} Invalid/skipped: <code>{len(cookies) - len(valid_cookies)}</code>\n\n"
            f"{get_emoji('🚀')} Starting validation...",
            parse_mode="HTML"
        )
        ...
        # Final response
        response = (
            f"{format_header('Bulk Check Complete', '📊')}\n"
            f"{SEP}\n"
            f"{get_emoji('📥')} <b>Processed:</b> <code>{len(results)}</code>\n"
            f"{get_emoji('✅')} <b>Valid:</b> <code>{valid_count}</code>\n"
            f"{get_emoji('❌')} <b>Invalid/Expired:</b> <code>{len(results) - valid_count}</code>\n\n"
            f"{get_emoji('🦇')} <b>Developer:</b> @OfficialAnnebella"
        )
...
async def cancel_bulk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"{get_emoji('❌')} <b>Bulk operation cancelled.</b>",
        parse_mode="HTML"
    )
    return ConversationHandler.END
...
async def admin_cookies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = (
        f"{format_header('Cookie Statistics', '🍪')}\n"
        f"{SEP}\n"
        f"{get_emoji('📊')} <b>Total:</b> <code>{stats['total']}</code>\n"
        f"{get_emoji('✅')} <b>Valid:</b> <code>{stats['valid']}</code>\n"
        f"{get_emoji('❌')} <b>Invalid:</b> <code>{stats['invalid']}</code>\n"
        f"{get_emoji('⏰')} <b>Expired:</b> <code>{stats['expired']}</code>\n"
        f"{get_emoji('⏳')} <b>Pending:</b> <code>{stats['pending']}</code>\n\n"
        f"{format_header('Tier Distribution', '⭐')}\n"
        f"  {get_emoji('⭐')} Premium: <code>{stats['tiers']['Premium']}</code>\n"
        f"  {get_emoji('📺')} Standard: <code>{stats['tiers']['Standard']}</code>\n"
        f"  {get_emoji('📱')} Basic: <code>{stats['tiers']['Basic']}</code>"
    )
    await update.message.reply_text(response, parse_mode="HTML")
...
async def admin_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(f"{get_emoji('✅')} Yes, clear all", callback_data="clear_confirm"),
            InlineKeyboardButton(f"{get_emoji('❌')} Cancel", callback_data="clear_cancel")
        ]
    ])
    await update.message.reply_text(
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("upload_"):
        # Trigger bulk upload
        await query.edit_message_text(
            f"{get_emoji('📤')} <b>Bulk Upload</b>\n\n"
            f"Please use <code>/bulk</code> command to upload a .txt file.",
            parse_mode="HTML"
        )
        return
...
def main():
            ...
            BULK_CHECK_STATE: [
                ...
                MessageHandler(filters.TEXT & ~filters.COMMAND,
                               lambda update, context: update.message.reply_text(
                                   f"{get_emoji('❌')} Please send a .txt file or /cancel.",
                                   parse_mode="HTML"
                               ))
            ]
        },
    ...
    # Message handler for direct cookie paste
