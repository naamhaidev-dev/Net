...
def split_cookie_blocks(content: str) -> List[str]:
    """
    Split content by one or more blank lines (lines with only whitespace).
    Returns a list of non-empty blocks, each stripped.
    """
    # Split on \n followed by zero or more whitespace then \n
    blocks = re.split(r'\n\s*\n', content.strip())
...
async def handle_cookie_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Split into cookie blocks (separated by blank lines)
        blocks = split_cookie_blocks(content)
        if not blocks:
            await msg.edit_text(
                f"{get_emoji('❌')} <b>Empty file</b>\nNo cookie blocks found.",
                parse_mode="HTML"
            )
            return

        if len(blocks) > MAX_BULK_CHECK:
            await msg.edit_text(
                f"{get_emoji('⛔️')} <b>Too Many Cookies</b>\n"
                f"Max: <code>{MAX_BULK_CHECK}</code>\n"
                f"Found: <code>{len(blocks)}</code>",
                parse_mode="HTML"
            )
            return

        # Validate each block and add to DB
        added = db.add_cookies_bulk(blocks, user_id)
        valid_cookies = [c for c in added if c.get("status") == "added"]

        if not valid_cookies:
            await msg.edit_text(
                f"{get_emoji('❌')} <b>No Valid Cookies</b>\n\n"
                f"All {len(blocks)} blocks were invalid or skipped.",
                parse_mode="HTML"
            )
            return

        await msg.edit_text(
            f"{get_emoji('✅')} <b>Added {len(valid_cookies)} cookie(s)</b>\n"
            f"{get_emoji('❌')} Invalid/skipped: <code>{len(blocks) - len(valid_cookies)}</code>\n\n"
